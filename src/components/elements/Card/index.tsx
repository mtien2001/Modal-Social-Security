...
import {ICardLeftProps, ICardProps, ICardRightProps} from "./types";
...
const Warranty = (props: ICardRightProps): JSX.Element => {
  const { labels, textMoney, textDescMoney, textDesc } = props
...
export const Card = (props: ICardProps): JSX.Element => {
  const { labels, textMoney, title, image, textDesc, textDescMoney, isDisabled } = props;
  return (
    <Flex
      margin={"0 0 12px 0"}
      width={"100%"}
    ...
    >
      ...
      {isDisabled ?
          <NotWarranty />
          :
          <Warranty labels={labels} textMoney={textMoney} textDescMoney={textDescMoney} textDesc={textDesc} />}
    </Flex>
  );
};
